...
var Jet =
{
	...
	new: func(n, running = 0, idle_throttle = 0.01, max_start_n1 = 5.21, start_threshold = 3, spool_time = 4, start_time = 0.02, shutdown_time = 4)
	{
        ...
        m.sip.setBoolValue();
		m.throttle = props.globals.getNode("systems/turbine/throttle", 1);
		m.throttle.setDoubleValue(0);
		m.throttle_lever = props.globals.getNode("systems/turbine/throttle-demand", 1);
...
#     spool_time = 4, start_time = 0.02, shutdown_time = 4)
      ...
      #var mainturbine = Jet.new(0 , 0 , 0.005 , 12.8 , 10 , 1 , 0.05 , 1);
	  var mainturbine = Jet.new(0 , 0 , 0.00 , 6 , 0.05 , 8 , 9 , 4);
	  mainturbine.init();
